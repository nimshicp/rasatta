...
import { useCallback, useEffect, useMemo, useRef, useState, forwardRef, useImperativeHandle, ReactNode } from 'react';
...
export interface DepthCarouselProps {
  ...
  onChange?: (index: number, item: DepthCarouselItem | string) => void;
  className?: string;
}
...
const normalizeItem = (it: DepthCarouselItem | string): DepthCarouselItem => (typeof it === 'string' ? { image: it, alt: '' } : it);

const DepthCarousel = forwardRef<any, DepthCarouselProps>(({
  items = DEFAULT_ITEMS,
  cardWidth = 300,
  cardHeight = 380,
  radius = 18,
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver(entries => {
      ...
      const cfg = cfgRef.current;
      const needed = cfg.cardWidth + Math.abs(cfg.spread) + 40;
      scaleRef.current = clamp(w / needed, 0.85, 1.2);
      layout(posRef.current);
    });
    ro.observe(root);
  ...
  }, [layout, setFocus, disableInteraction]);
  ...
  return (
    <div
      ...
      onPointerMove={disableInteraction ? undefined : onPointerMove}
      onPointerUp={disableInteraction ? undefined : onPointerEnd}
      onPointerCancel={disableInteraction ? undefined : onPointerEnd}
      onKeyDown={disableInteraction ? undefined : onKeyDown}
    >
